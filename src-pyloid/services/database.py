...
class DatabaseService:
    def __init__(self, db_path: Optional[Path] = None):
        ...
        self._init_db()
        # Recording persistence lives in its own domain module (see
        # services/recording/repository.py and CONTEXT.md "Recording").
        # The delegating methods below stay for backward compatibility.
        self.recordings = RecordingsRepository(self)
    ...
    # (services/recording/repository.py). These thin delegates remain so
    # existing callers (server.py, tests) keep working unchanged.

    def create_recording(self, title: str, sources: list[str]) -> int:
        return self.recordings.create_recording(title, sources)

    def get_recording(self, recording_id: int, include_segments: bool = False) -> Optional[dict]:
        return self.recordings.get_recording(recording_id, include_segments=include_segments)
    ...
    def list_recordings(
        ...
        return self.recordings.list_recordings(limit=limit, offset=offset, search=search)

    def update_recording(self, recording_id: int, **fields) -> None:
        return self.recordings.update_recording(recording_id, **fields)
    ...
    def set_recording_audio(
        ...
        return self.recordings.set_recording_audio(
            recording_id, audio_relpath, duration_ms, size_bytes, sample_rate, channels
        )

    def set_recording_recorder_state(self, recording_id: int, state: Optional[str]) -> None:
        return self.recordings.set_recording_recorder_state(recording_id, state)
    ...
    def update_transcript_status(
        ...
        return self.recordings.update_transcript_status(
            recording_id, status, progress=progress, error=error
        )
    ...
    def update_summary_status(
        ...
        return self.recordings.update_summary_status(
            recording_id, status, progress=progress, error=error, provider=provider
        )
    ...
    def set_recording_transcript(
        ...
        return self.recordings.set_recording_transcript(
            recording_id, transcript, language=language, model=model
        )

    def replace_recording_segments(self, recording_id: int, segments: list[dict]) -> None:
        return self.recordings.replace_recording_segments(recording_id, segments)

    def delete_recording(self, recording_id: int) -> None:
        return self.recordings.delete_recording(recording_id)

    def list_unfinished_recordings(self) -> list[dict]:
        return self.recordings.list_unfinished_recordings()
